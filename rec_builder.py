...
class RecBuilder():
    def __init__(self, path, image_size=(112, 112), output):
        self.path = path
        ...
        os.makedirs(path)
        self.writer = mx.recordio.MXIndexedRecordIO(os.path.join(path, f'{output}.idx'), 
                                                    os.path.join(path, f'{output}.rec'),
                                                    'w')
        self.meta = []
...
parser = argparse.ArgumentParser(description='create file rec for training')
parser.add_argument('--dataset-dir', default='', help='folder includes images')
parser.add_argument('--output', default='', help='Full path to the directory where our identified images should be saved.')

args = parser.parse_args()
data_dir = args.dataset_dir
output = args.output
recbuilder = RecBuilder(path=data_dir, output=output)